...
def total_loss(fmap_real, logits_fake, fmap_fake, wav1, wav2, sample_rate=24000):
    ...
    loss = torch.tensor([0.0], device='cpu', requires_grad=True)
    ...
    def train(epoch):
        ...
        for batch_idx, input_wav in enumerate(trainloader):
            ...
            if batch_idx % log_interval == 0:
                ...
                print(loss)
                torchaudio.save('./real.wav', input_wav[0].cpu().detach(), model.sample_rate, channels_first=True)
                torchaudio.save('./fake.wav', output[0].cpu().detach(), model.sample_rate, channels_first=True)
